...
def get_job(node, partition, cores, mem, account, time, odir, tmp):
    ...
    cmd = f"sbatch --output {odir}/tmp_node_script.out --parsable -A {account} --mem={mem} --time={time}:00:00 -p {partition} -c {cores} --job-name=intractv {odir}/tmp_node_script.sh  --comment={comment}"
